...
def build_task_text(events: list) -> str:
    """สร้างข้อความสรุปงานวันนี้ พร้อมเวลาและห้อง/สถานที่ (ถ้ามี)"""
    now = now_in_bangkok()
    ...
    if not events:
        return (
            f"📅 แจ้งเตือนตารางงาน\n\n"
            f"📆 วันที่ {thai_date}\n\n"
            f"ไม่มีตารางงานที่กำหนดไว้สำหรับวันนี้"
        )

    lines = [f"📅 แจ้งเตือนตารางงาน\n\n📆 วันที่ {thai_date}\n"]

    for event in events:
        title = event.get("summary", "(ไม่มีชื่อกิจกรรม)")
        ...
        if "dateTime" in start:
            ...
            if "dateTime" in end:
                en = datetime.datetime.fromisoformat(end["dateTime"]).astimezone(BANGKOK_TZ)
                en_str = en.strftime("%H:%M")
                if en_str != time_str:
                    time_str = f"{time_str} - {en_str}"
        else:
            time_str = "ทั้งวัน"
...
def main():
    ...
    # DEBUG: เช็คความยาวของ token (ไม่โชว์ค่าจริง) เพื่อดูว่า Secret ถูกอ่านเข้ามาถูกต้องไหม
    print(f"[DEBUG] ความยาวของ LINE_CHANNEL_ACCESS_TOKEN = {len(access_token)} ตัวอักษร", file=sys.stderr)
    print(f"[DEBUG] ตัวอักษร 6 ตัวแรกของ token = {access_token[:6]!r}", file=sys.stderr)
    print(f"[DEBUG] ตัวอักษร 6 ตัวสุดท้ายของ token = {access_token[-6:]!r}", file=sys.stderr)
